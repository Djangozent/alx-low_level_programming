...
/**
 * print_version - prins the version of an ELF header
 * @e_ident: poointer to an array containing the ELF version
 */
void print_version(unsigned char *e_ident)
{
	printf("  Version:			%d",
 ...
 *
 * Description: if the file cannot be be closed, exit code 98
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 *
 * Des: if the file is not an ELF file or the function fails, exit code 98
 */
